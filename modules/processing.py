...
def proc_or_opt(p: StableDiffusionProcessing | None, key: str, opts_key: str | None = None):
    ...
    return getattr(shared.opts, opts_key, None)
    ...
    for i, sample in enumerate(samples):
        ...
        if not shared.state.interrupted and not shared.state.skipped:
            ...
            if p.detailer_enabled:
                ...
                if not p.do_not_save_samples and proc_or_opt(p, 'save_images_before_detailer'):
                    info = create_infotext(p, p.prompts, p.seeds, p.subseeds, index=i)
                    images.save_image(image, path=p.outpath_samples, basename="", seed=p.seeds[i], prompt=p.prompts[i], extension=proc_or_opt(p, 'samples_format'), info=info, p=p, suffix="-before-detailer")
            ...
            if p.color_corrections is not None and i < len(p.color_corrections):
                p.ops.append('color')
                if not p.do_not_save_samples and proc_or_opt(p, 'save_images_before_color_correction'):
                    image_without_cc = apply_overlay(image, p.paste_to, i, p.overlay_images)
                    info = create_infotext(p, p.prompts, p.seeds, p.subseeds, index=i)
                    images.save_image(image_without_cc, path=p.outpath_samples, basename="", seed=p.seeds[i], prompt=p.prompts[i], extension=proc_or_opt(p, 'samples_format'), info=info, p=p, suffix="-before-color-correct")
        ...
        info = create_infotext(p, p.prompts, p.seeds, p.subseeds, index=i)
        if proc_or_opt(p, 'samples_save') and not p.do_not_save_samples and p.outpath_samples is not None:
            images.save_image(image, p.outpath_samples, "", p.seeds[i], p.prompts[i], proc_or_opt(p, 'samples_format'), info=info, p=p) # main save image
    ...
    with devices.inference_context():
        ...
        for n in range(p.n_iter):
            ...
            if shared.state.interrupted:
                ...
                _keep = proc_or_opt(p, 'keep_incomplete')
        ...
        _return_grid = proc_or_opt(p, 'return_grid')
        _grid_save = proc_or_opt(p, 'grid_save')
        if (_return_grid or _grid_save) and (not p.do_not_save_grid) and (len(output_images) > 1):
            if images.check_grid_size(output_images):
                ...
                if _grid_save:
                    images.save_image(grid, p.outpath_grids, "", p.all_seeds[0], p.all_prompts[0], proc_or_opt(p, 'grid_format'), info=grid_info, p=p, grid=True) # main save grid
